...
class Basic_PT(hk.Module):
    ...
    def __call__(self, node_tensors, hidden, **unused_kwargs):
        block = PTTransformerLayer(self.NS, self.H, self.HS, self.NHS, self.dropout_rate,
                                   name=self.name)
        node_tensors = block(node_tensors)

        return node_tensors


class PTTransformerLayer(hk.Module):
    def __init__(
            self,
            NS: int,
            H: int,
    ...
    def __call__(self, node_tensors):
        NA = PTAttentionLayer(self.NS, self.H, self.HS)
        ...
        attw_node_tensors = NA(node_tensors)
...
class PTAttentionLayer(hk.Module):
    def __init__(self, NS, H, HS):
        super().__init__()
        self.NS = NS
        self.H = H
        self.HS = HS
    ...
    def __call__(self, node_tensors):
        ...
        nK = jnp.transpose(nK, (0, 1, 3, 2))
        QK = jnp.matmul(nQ, nK)

        QK = QK * self.scale
        att_dist = jax.nn.softmax(QK, axis=-1)

        new_nodes = jnp.matmul(att_dist, nV)
...
class RTAttentionLayer(hk.Module):
    ...
    def __call__(self, node_tensors, edge_tensors, graph_tensors):
        ...
        if self.graph_vec == 'att':
            ...
            K = eK + jnp.reshape(nK, (B, H, 1, N, HS)) + jnp.reshape(gK, (B, H, 1, 1, HS))
        else:
        ...
        if self.graph_vec == 'att':
            v2 = eV + jnp.reshape(nV, (B, H, 1, N, HS)) + jnp.reshape(gV, (B, H, 1, 1, HS))
        else:
